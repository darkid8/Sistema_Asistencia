...
describe('login screen', () => {
  ...
  it('stores the session profile but never the token when credentials are valid', async () => {
    vi.stubGlobal(
      'fetch',
    ...
    await waitFor(() => {
      const stored = window.localStorage.getItem('workshop.session');
      expect(stored).toContain('Administrador del taller');
      // The real fix under test: the token the backend returned in the body
      // must never reach localStorage, only the HttpOnly cookie the browser
      // already holds carries it.
