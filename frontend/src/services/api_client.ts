...
export interface RequestOption {
  ...
  body?: unknown;
  /**
   * @deprecated The session credential now travels as an HttpOnly cookie the
   * browser attaches automatically (see `credentials: 'include'` below), so
   * this is no longer read. Session-consuming call sites still pass it
   * through unchanged; that costs nothing and keeps this change contained to
   * the HTTP client instead of touching every screen and service file.
   */
  token?: string | null;
  signal?: AbortSignal;
}
...
export async function request<T>(path: string, option: RequestOption = {}): Promise<T> {
  ...
  try {
    const response = await fetch(API_BASE + path, {
      ...
      // The session lives in an HttpOnly cookie now instead of a header this
      // code builds by hand, so the browser must be told to attach it.
      credentials: 'include',
    });
    if (!response.ok) {
