...
func TestVehicleRejectsAnHTMLPayloadInTheModel(t *testing.T) {
	_, err := domain.NewVehicle(
		"vehicle-1", "customer-1", "ABC123", "VIN1", "Mazda", "<img src=x onerror=alert(1)>", 2019, time.Now(),
	)
	if !errors.Is(err, domain.ErrInvalidInput) {
		t.Fatalf("a model carrying HTML markup must be rejected, got %v", err)
	}
}
