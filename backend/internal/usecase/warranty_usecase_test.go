...
func (f *fakeWarrantyRepository) FindByInterventionAndKind(
	_ context.Context, interventionID string, kind domain.WarrantyKind,
) (domain.Warranty, error) {
	for _, item := range f.warranty {
		if item.InterventionID == interventionID && item.Kind == kind {
			return item, nil
		}
	}
	return domain.Warranty{}, domain.ErrNotFound
...
func TestIssueRejectsTheSameKindTwiceOverTheSameIntervention(t *testing.T) {
	warranties := newFakeWarrantyRepository()
	useCase := usecase.NewWarrantyUseCase(
		warranties, newFakeInterventionRepository(t, "intervention-1"), sequentialID(), fixedClock(),
	)
	if _, err := useCase.Issue(context.Background(), "intervention-1", domain.WarrantyLabor, 12); err != nil {
		t.Fatalf("issuing the first LABOR warranty must be accepted: %v", err)
	}

	if _, err := useCase.Issue(context.Background(), "intervention-1", domain.WarrantyLabor, 12); !errors.Is(err, domain.ErrConflict) {
		t.Fatalf("a second LABOR warranty over the same intervention must be a conflict, got %v", err)
	}
}

func TestIssueAllowsDifferentKindsOverTheSameIntervention(t *testing.T) {
	warranties := newFakeWarrantyRepository()
	useCase := usecase.NewWarrantyUseCase(
		warranties, newFakeInterventionRepository(t, "intervention-1"), sequentialID(), fixedClock(),
	)
	if _, err := useCase.Issue(context.Background(), "intervention-1", domain.WarrantyLabor, 12); err != nil {
		t.Fatalf("issuing the LABOR warranty must be accepted: %v", err)
	}
	if _, err := useCase.Issue(context.Background(), "intervention-1", domain.WarrantyPart, 6); err != nil {
		t.Fatalf("a PART warranty on the same intervention is a different kind and must be accepted: %v", err)
	}
}
