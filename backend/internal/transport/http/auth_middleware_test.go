...
func TestAuthMiddlewareAcceptsAValidSessionCookie(t *testing.T) {
	reached := false
	handler := authMiddleware(testIssuer(), testClock())(protectedProbe(&reached))

	request := httptest.NewRequest(http.MethodGet, "/api/customer", nil)
	request.AddCookie(&http.Cookie{Name: sessionCookieName, Value: tokenFor(t, domain.RoleAdministrator)})
	recorder := httptest.NewRecorder()
	handler.ServeHTTP(recorder, request)
...
func TestAuthMiddlewarePrefersTheCookieOverATamperedHeader(t *testing.T) {
	reached := false
	handler := authMiddleware(testIssuer(), testClock())(protectedProbe(&reached))
	...
	if recorder.Code != http.StatusOK || !reached {
		t.Fatalf("a valid cookie must be used even when the header is garbage, got %d reached=%v", recorder.Code, reached)
	}
}
