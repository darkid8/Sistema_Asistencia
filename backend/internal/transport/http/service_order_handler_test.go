...
func newOrderHandler(orders *fakeOrderStore) ServiceOrderHandler {
	return NewServiceOrderHandler(usecase.NewServiceOrderUseCase(
		orders, newFakeVehicleStore("vehicle-1"), &fakeAssignmentStore{}, &fakeTechnicianStore{},
...
func TestAdvanceIsForbiddenForATechnicianNotAssignedToTheOrder(t *testing.T) {
	orders := newFakeOrderStore(orderFixture(t, "order-1", domain.StatusReceived))
	...
	technicians := newFakeTechnicianStore(t, "technician-2", "user-1")
	handler := NewServiceOrderHandler(usecase.NewServiceOrderUseCase(
	...
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("a technician who does not hold the order must be refused, got %d body %s", recorder.Code, recorder.Body.String())
	}
	if orders.writes != 0 {
		t.Fatalf("a refused advance must write nothing, got %d writes", orders.writes)
	}
}
