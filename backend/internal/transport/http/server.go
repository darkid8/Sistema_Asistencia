...
func NewServer(dependency Dependency) *http.Server {
	...
	authHandler := NewAuthHandler(usecase.NewAuthenticateUser(userRepository, issuer, now), now, settings.CookieSecure)
	...
	orderUseCase := usecase.NewServiceOrderUseCase(
		orderRepository, vehicleRepository, assignmentRepository, technicianRepository, newID, now,
	)
	...
	root.HandleFunc("POST /api/session/sign-out", authHandler.SignOut)
