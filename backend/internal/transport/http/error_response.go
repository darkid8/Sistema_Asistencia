...
func classify(err error) (int, string, string) {
	...
	case errors.Is(err, domain.ErrTooManyRequests):
		return http.StatusTooManyRequests, "too_many_requests", "Demasiados intentos fallidos. Intente de nuevo en unos minutos."
	default:
...
func conflictMessage(err error) string {
	...
	case strings.Contains(text, "warranty was already issued"):
		return "Ya existe una garantia de este tipo para esta intervencion."
	default:
...
func invalidInputMessage(err error) string {
	...
	case strings.Contains(text, "unsafe characters"):
