...
func (f *fakeWarrantyStore) FindByInterventionAndKind(
...
func TestIssueWarrantyRejectsTheSameKindTwiceInSpanish(t *testing.T) {
	handler := newWarrantyHandler(t, newFakeWarrantyStore())
	first := issueWarranty(t,
		handler, `{"interventionId":"intervention-1","kind":"LABOR","coverageMonthCount":12}`, domain.RoleAdministrator,
	)
	if first.Code != http.StatusCreated {
		t.Fatalf("the first warranty must be accepted, got %d body %s", first.Code, first.Body.String())
	}

	second := issueWarranty(t,
		handler, `{"interventionId":"intervention-1","kind":"LABOR","coverageMonthCount":12}`, domain.RoleAdministrator,
	)
	if second.Code != http.StatusConflict {
		t.Fatalf("a duplicate warranty must answer 409, got %d body %s", second.Code, second.Body.String())
	}
	var payload errorPayload
	if err := json.Unmarshal(second.Body.Bytes(), &payload); err != nil {
		t.Fatalf("the response must be an error payload: %v", err)
	}
	if payload.Message != "Ya existe una garantia de este tipo para esta intervencion." {
		t.Fatalf("expected the Spanish duplicate-warranty message, got %q", payload.Message)
	}
}
