...
// FindByInterventionAndKind reads the warranty of one kind issued over an
// intervention, used to refuse issuing the same coverage twice.
func (r WarrantyRepository) FindByInterventionAndKind(
	ctx context.Context, interventionID string, kind domain.WarrantyKind,
) (domain.Warranty, error) {
	queryCtx, cancel := context.WithTimeout(ctx, r.timeout)
	defer cancel()

	var warranty domain.Warranty
	var scannedKind string
	var createdAt time.Time
	err := r.database.QueryRowContext(
		queryCtx, "SELECT "+warrantyColumn+" FROM warranty WHERE intervention_id = ? AND warranty_kind = ?",
		interventionID, string(kind),
	).Scan(
		&warranty.ID, &warranty.InterventionID, &scannedKind, &warranty.CoverageMonthCount,
		&warranty.IssuedAt, &warranty.ExpirationDate, &createdAt,
	)
	if err != nil {
		return domain.Warranty{}, translate(err)
	}
	warranty.Kind = domain.WarrantyKind(scannedKind)
	return warranty, nil
}
